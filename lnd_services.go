...
// and make back off if the failure is due to lnd currently being locked.
// Otherwise, it will fail fast on any errors returned. We use the raw ln client
// so that we can set specific grpc options we need to wait for lnd to be ready.
